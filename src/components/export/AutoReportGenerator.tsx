  ...
  const generateReportNow = async () => {
    ...
    try {
      ...
      if (config.frequency === 'monthly') {
        ...
        filename = `Rapport_${template.name}_${month}_${year}.${config.format}`;
        ...
        filename = `Rapport_${template.name}_Q${quarter}_${year}.${config.format}`;
      } else {
        await exportService.generateYearlyReport({ 
          year, 
          format: config.format
        });
        filename = `Rapport_${template.name}_${year}.${config.format}`;
      }
