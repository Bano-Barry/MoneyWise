...
export const getTransactions = async (): Promise<Transaction[]> => {
  try {
    ...
    console.log('Réponse brute de l\'API transactions:', response.data);
...
export const getRecentTransactions = async (limit: number = 5): Promise<Transaction[]> => {
  try {
    ...
    console.log('Réponse brute de l\'API transactions récentes:', response.data);
  ...
  try {
    ...
    const transactionData = {
      ...
      date: new Date().toISOString()
            ...
            console.log('Données complètes envoyées à l\'API:', transactionData);
            console.log('Type de catégorie dans la requête:', transactionData.type);
            console.log('CategoryId dans la requête:', transactionData.categoryId);
