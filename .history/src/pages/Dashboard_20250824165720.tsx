...
export default function Dashboard() {
  ...
  const [trackingPeriod, setTrackingPeriod] = useState<'jour' | 'semaine' | 'mois'>('jour');


  const expensePercentage = totalIncome > 0 ? Math.round((totalExpenses / totalIncome) * 100) : 0;

  // -- Period-based transactions tracking --
  function getPeriodKey(date: Date, period: 'jour' | 'semaine' | 'mois') {
    if (period === 'jour') {
    ...
    if (period === 'semaine') {
      ...
      temp.setDate(date.getDate() - day + 1);
      const weekNum = Math.ceil((temp.getDate() + 6) / 7);
      return `${temp.getFullYear()}-S${weekNum}`;
    }
    return `${date.getFullYear()}-${date.getMonth() + 1}`;
  }
  const trackedDataMap = new Map<string, { income: number; expenses: number }>();
  currentMonthTransactions.forEach((t) => {
    const date = new Date(t.date);
    const key = getPeriodKey(date, trackingPeriod);
    if (!trackedDataMap.has(key)) trackedDataMap.set(key, { income: 0, expenses: 0 });
    const value = trackedDataMap.get(key)!;
    if (t.type === 'income') value.income += t.amount;
    else if (t.type === 'expense') value.expenses += t.amount;
  });
  const trackedData = Array.from(trackedDataMap.entries()).map(([period, vals]) => ({
    period,
    ...vals,
  }));
  trackedData.sort((a, b) => a.period.localeCompare(b.period));
  // -- End period-based tracking --
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
            <span className="mr-2">Évolution du mois en </span>
            <button
              className={`rounded px-2 py-1 mx-1 text-xs font-semibold ${
                trackingPeriod === 'jour' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-900'
              }`}
              onClick={() => setTrackingPeriod('jour')}
            >
              Jour
            </button>
            <button
              className={`rounded px-2 py-1 mx-1 text-xs font-semibold ${
                trackingPeriod === 'semaine' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-900'
              }`}
              onClick={() => setTrackingPeriod('semaine')}
            >
              Semaine
            </button>
            <button
              className={`rounded px-2 py-1 mx-1 text-xs font-semibold ${
                trackingPeriod === 'mois' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-900'
              }`}
              onClick={() => setTrackingPeriod('mois')}
            >
              Mois
            </button>
          </h3>
