...
export default function Dashboard() {
  ...
  const getProjectedBalance = () => {
    // Simple projection: assume current month income and expenses continue
    const projectedIncome = totalIncome;
    const projectedExpenses = totalExpenses;
    return balance + projectedIncome - projectedExpenses;
  };

  // Ajoute cet état pour afficher/masquer le pourcentage
  const [showExpensePercentage, setShowExpensePercentage] = useState(false);

  // Calcul du pourcentage dépenses/revenus (évite division par zéro)
  const expensePercentage = totalIncome > 0 ? Math.round((totalExpenses / totalIncome) * 100) : 0;

  return (const [variationView, setVariationView] = useState<'jour' | 'semaine' | 'mois'>('jour');

// Helper pour grouper{/* Suivi des revenus et dépenses (variation) */}
